...
if (isset($_POST['update_product'])) {
   ...
   $update_product = $conn->prepare("UPDATE `products` SET name = ?,farmer_email=?, category = ?, details = ?, price = ? WHERE id = ?");
   $update_product->execute([$name,$farmer_email, $category, $details, $price, $pid]);
      ...
      if ($select_products->rowCount() > 0) {
         while ($fetch_products = $select_products->fetch(PDO::FETCH_ASSOC)) {
            ...
            <form action="" method="post" enctype="multipart/form-data">
               ...
               <input type="email" name="farmer_email" placeholder="enter your email" required class="box"
                  value="<?= $fetch_products['farmer_email']; ?>">
